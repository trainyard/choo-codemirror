...
exports.create = ({namespace}, options) => {
  ...
  defer(() => {
    ...
    editor.on('change', debounce(broadcast('change'), 100))
    // editor.on('focus', debounce(broadcast('focus'), 3000))
    // editor.on('blur', debounce(broadcast('blur'), 3000))
    editor.on('scroll', debounce(broadcast('scroll'), 100))
  })
  return node
}
