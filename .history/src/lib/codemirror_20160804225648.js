...
exports.create = ({namespace}, options) => {
  ...
  defer(() => {
    ...
    editor.on('change', debounce(broadcast('change')))
    editor.on('focus', debounce(broadcast('focus')))
    editor.on('blur', debounce(broadcast('blur')))
    editor.on('scroll', debounce(broadcast('scroll')))
  })
  return node
}
